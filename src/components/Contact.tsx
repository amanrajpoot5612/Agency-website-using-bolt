...
export const Contact = () => {
  ...
  const handleSubmit = async (e: FormEvent) => { e.preventDefault(); if (await submit(form, turnstileToken)) setForm({ ...form, name: '', email: '', phone: '', message: '' }); setTurnstileToken(''); };
  return <section id="contact" className="section contact-section"><div className="shell contact-grid">
    ...
    <TurnstileWidget onToken={setTurnstileToken} /><div className="form-submit"><p aria-live="polite">{status === 'success' ? 'Thanks — your message is on its way.' : status === 'error' ? error : 'No hard sell. Just a useful first conversation.'}</p><button disabled={status === 'loading' || !turnstileToken}>{status === 'loading' ? 'Sending…' : 'Send inquiry'}<ArrowUpRight size={17}/></button></div></form>
  </div></section>;
};
