...
interface PricingPlan { name: string; price: string; description: string; features: string[] }

export default function PricingPage() {
  const plans = data.pricing.plans as PricingPlan[];

  return (
    <PageLayout>
      <Seo title="Pricing" description="Transparent INR project estimates for portfolios, landing pages, and business websites." path="/pricing" />
      <section className="bg-navy-950 pb-20">
        <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 lg:px-8">
          ...
          <div className="mt-14 grid gap-6 lg:grid-cols-3">
            {plans.map((plan) => (
              <div key={plan.name} className="rounded-[2rem] border border-cyan-400/10 bg-navy-950/70 p-8 shadow-2xl shadow-cyan-500/5 transition hover:-translate-y-1">
                <p className="text-sm uppercase tracking-[0.32em] text-cyan-300">{plan.name}</p>
